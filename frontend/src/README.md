...
| `components/chatbot/` | `ChatInput`, `ChatMessages` (react-markdown + remark-gfm rendering, plus thumbs up/down under each finished reply — `ChatBot.handleFeedback` → `api.sendFeedback`, kept in memory for the page's life), `TaskSection` (one collapsible step: an executed node, or the plan diagram) |
