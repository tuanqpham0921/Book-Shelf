...
- `stores/` — repository pattern; routes/workflows never touch sessions directly.
  ...
  **Who owns the session, and for how long.** A store is short-lived by
  ...
  `feedback_store.py` (review upsert — the /review page's reviews and the
  chat's own feedback alike).
...
| `feedback` | One review per (chat_id, session_id), upserted whole: the /review page's reviews and the chat's own feedback (whose session is the one that produced the run). FK `chat_id` → `chat_runs`, CASCADE |
