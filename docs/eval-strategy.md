...
the step's own request builder sent straight to OpenAI. Two exist: triage's router
(`backend/evals/triage/`, `make eval-routing`, 2026-09-25 — it replaced the query
decomposition eval along with the decomposition) and the message check
(`backend/evals/validation/`, `make eval-validation`, 2026-09-25); a node's `*Args`
parse would follow them under `backend/evals/nodes/`.
